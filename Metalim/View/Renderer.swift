...
class Renderer: NSObject, MTKViewDelegate {
    ...
    init(_ parent: ContentView, scene: GameScene) {
        ...
        self.mesh = ObjMesh(device: metalDevice, allocator: allocator, filename: "cube")
        self.material = Material(device: metalDevice, allocator: materialLoader, filename: "texture")
    ...
    func draw(in view: MTKView) {
        ...
        cameraData.view = Matrix44.create_lookat(eye: scene.player.position, target: scene.player.position + scene.player.forwards, up: scene.player.up)
        cameraData.projection = Matrix44.create_perspective_projection(fovy: 45, aspect: 800/600, near: 0.1, far: 10)
